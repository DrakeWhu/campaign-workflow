...
from campaign_workflow.core.path_safety import (
    PathSafetyError,
    validate_existing_file_inside_case,
)
from campaign_workflow.core.state import now_utc


SUPPORTED_REDUCED_OUTPUT_KINDS = {"csv", "file"}
...
def validate_reduced_output(
    *,
    ...
    This validator is intentionally generic. All outputs must be safe,
    case-local regular files with an allowed suffix. CSV outputs also require
    readable tabular content; generic ``file`` outputs only require the
    configured file existence/minimum-size contract. It does not inspect plot
    pixels or interpret guiding/capillary physics.
    """
    ...
    min_rows = (
        _nonnegative_int(output.get("min_rows", 1), "min_rows") if kind == "csv" else 0
    )
    required_columns = (
        _string_list(output.get("required_columns", []), "required_columns")
        if kind == "csv"
        else []
    )
    min_bytes = (
        _positive_int(output.get("min_bytes", 1), "min_bytes")
        if kind == "file"
        else None
    )
    ...
    try:
        resolved_path = validate_existing_file_inside_case(
            case_dir, raw_path, label=f"reduced output {name!r}"
        )
    ...
    if resolved_path is not None:
        try:
            ...
            if suffix not in allowed_suffixes:
                errors.append(
                    f"invalid suffix for reduced output {name!r}: {suffix!r}; allowed={allowed_suffixes}"
                )
            if file_entry["size_bytes"] <= 0:
                errors.append(f"empty reduced output {name!r}: {raw_path}")
            elif min_bytes is not None and file_entry["size_bytes"] < min_bytes:
                errors.append(
                    f"reduced output {name!r} requires at least {min_bytes} byte(s), "
                    f"found {file_entry['size_bytes']}"
                )
        except Exception as exc:
            errors.append(f"failed to inspect reduced output {name!r}: {exc}")

    if kind == "csv" and resolved_path is not None and not errors:
    ...
    *,
    ...
    try:
        with path.open("r", encoding="utf-8-sig", newline="") as f:
            ...
            if missing_columns:
                errors.append(
                    f"CSV missing required column(s): {missing_columns}; columns={columns}"
                )
    ...
    if row_count < min_rows:
        errors.append(
            f"CSV requires at least {min_rows} data row(s), found {row_count}"
        )
...
def _allowed_suffixes(output: dict[str, Any], kind: str) -> list[str]:
    if "allowed_suffixes" in output:
        raw = output["allowed_suffixes"]
    elif kind == "csv":
        raw = [".csv"]
    else:
        raise ValueError(
            "file reduced outputs must declare a non-empty allowed_suffixes list"
        )
...
def _failure(
    *, output_name: str, output_kind: str, path: str, errors: list[str]
) -> dict[str, Any]:
    return {
        "schema_version": 1,
